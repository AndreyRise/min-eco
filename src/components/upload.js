...
import uploadImg from '../img/HelloPage/upload.jpg';
...
function UploadComp(props) {  
  ...
  const genres = [
    'Hip-Hop/Rap','Pop','R&B','Dance','Alternative','Rock','Blues','African','Arabic','Asian','Brazilian','Children Music','Classical','Country','Easy Listening','Electronic','Folk','Indian','Jazz','Latin','Metal','Reggae','World Music / Regional Folklore','Другое'
  ];  
  ...
  const deleteSong = (id) => {
    console.log(songList.findIndex(song => song.id == id));
    console.log(songList.find(song => song.id == id));
    const index = songList.findIndex(song => song.id == id);
    let updSongList = songList;
    updSongList.splice(index,1);
    console.log(updSongList);
    ...
    <div className='flex flex-col w-full gap-3'>
      ...
      <div className="divWrap flex flex-col gap-5 sm:flex-row">
        ...
        <div className="flex flex-col flex-1 gap-1">
          ...
          <h2 className="divText mb-0">Ссылка на активную соц.сеть артиста</h2>
          <input type='proofUrl' className='focus:outline-none w-full h-10 rounded-xl px-3 border' onChange={e=>setSoc2(e.target.value)} value={soc2} placeholder='Пример: instagram'/>
        </div>
      </div>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
        ...
        <div className="flex flex-col w-full items-start justify-center">
          <button className='text-sm rounded-full bg-sky-500  py-2 px-4 text-center font-semibold text-white transition-all hover:bg-sky-800' onClick={() => setIsOpen(true)}>Добавить трек</button>
        </div>
      </div>
    ...
    <Dialog.Panel className="max-h-[100%] w-max min-w-[300px] sm:min-w-[500px] rounded-lg">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-row gap-3 justify-center sm:justify-end mb-5">
          ...
          <button className='text-sm rounded-full bg-sky-500 py-2 px-4 text-center font-semibold text-white transition-all hover:bg-sky-800 w-[130px]' onClick={(e) => {songToArray();setIsOpen(false);clearSongInfo()}}>Сохранить</button>
        </div>
      </div>
    </Dialog.Panel>
  </Dialog>
