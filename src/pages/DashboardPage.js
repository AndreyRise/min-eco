    ...
    return (<>
      <div className="flex flex-col overflow-auto justify-between">
        ...
        <ul className="px-4 flex flex-2 flex-col justify-self-center">
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/main')}}>{svg.home}Главная</li>
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/upload')}}>{svg.upload}Загрузить релиз</li>
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/catalog')}}>{svg.catalog}Каталог</li>
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/links')}}>{svg.links}Линки</li>
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/finances')}}>{svg.finances}Финансы</li>
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/promo')}}>{svg.promo}Промо</li>
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/faq')}}>{svg.questionMark}FAQ</li>
          <li className="navBarBtn" onClick={()=>{setActionDetect(!actionDetect); navigate('/dashboard/settings')}}>{svg.gear}Настройки</li>
    ...
    {userStore.user.approved?<>
      ...
      <div className="dashboardNavbar flex min-h-[90vh] h-min w-full pt-10 lg:p-0 ">
        <Routes>
          ...
          <Route path="/upload" element={<Upload authHandler={authHandler}/>} />
